...
if os.path.exists(p):
    ...
    for layer in range(params.n_blocks):
        ...
        clique_lists.append(simplex.find_high_weight_faces(decomp.internal_correlations[layer],
                                                           decomp.ds_emb[layer], n_cliques_per_vertex=2, n_max_collect_per_vertex=4))
...
# I think pkl here fails b/c numpy... we can do smthng else
# Use numpy.save
# TODO: with all done file flag instead
if os.path.exists("face_corr_0.npy"):
	face_corr = [
		np.load(f"face_corr_{i}.npy") for i in range(5) # TODO: idk what range
	]
else:
	face_corr = simplex.face_correlation_lattice(decomp.internal_correlations, clique_lists)
	for i, corr in enumerate(face_corr):
		np.save(f"face_corr_{i}.npy", corr)
...
import numpy as np
np.sort(face_corr[0][0].flatten())[::-1][:100]
...
G = graph.GraphOfCorrs(face_corr[:N_LAYERS], corr_cutoff=0.3)
# G = pickle.load(open("tmp_G.pkl", "br"))
# 
# path_inds = graph.top_k_dag_paths(face_corr[:4], layer=0, neuron=11, k=5, corr_cutoff=0.1, all_disjoint=True)
...
# Alt, find paths with support! (non-trivially)
path_inds = graph._top_k_dag_paths(r, self.source, self.sink, self.graph_idx_to_node_idx,
                         self.node_idx_to_graph_idx, self.most_pos_per_layer,
                         self.n_layers, layer, neuron, k, all_disjoint=True)
...
k = 10
def get_paths_per_layer(layer: int):
    n_cliques = face_corr[layer].shape[0]
    all_paths_inds = []
    # TODO: you can probs/ defo || this...
    for n in range(n_cliques):
        all_paths_inds += G.get_top_k_paths(layer, n, k)
    return all_paths_inds

layer_0_path_inds = get_paths_per_layer(0)
...
layer_0_paths = [
    [ 
			clique_lists[layer][n][1] for layer, n in enumerate(path_idx[0])
    ] for path_idx in layer_0_path_inds
]
layer_0_paths
...
# BTW! A problem here is that this does not give us **the best** paths
# We can probably do *another* sort based off of the highest eq contenders here
def get_feature_candidates(features: List[List[List[int]]], n_candidates=2_048):
    candidates: List[List[int]] = []

    # Not the most sophisticated but okay
    n_times_seen = [np.zeros(params.model_n_dims * 2) for i in range(params.n_blocks)]

    def score_by_n_times_seen(feat: List[List[int]]):
        r = 0
        for layer, ns in enumerate(feat):
            for n in ns:
                r += n_times_seen[layer][n]
        return r

    while len(candidates) < n_candidates:

        all_score = np.array([score_by_n_times_seen(f) for f in features])
        lowest_scores = np.argsort(all_score)
        # print(lowest_scores[:10])
        candidates.append(features[lowest_scores[0]])

        for layer, ns in enumerate(candidates[-1]):
            for n in ns:
                n_times_seen[layer][n] += 1
    return candidates
        
feature_paths = get_feature_candidates(layer_0_paths)
...
# path_idx = feature_paths[0]
# path = [
# 	clique_lists[layer][node] for layer, node in enumerate(path_idx)
# ]
# path
path = feature_paths[3]
path
