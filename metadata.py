...
class InterpParams:
    ...
    def get_and_prepare_correlation_save_tag(self, prepend: str):
        ...
        if not os.path.exists(f'metadata/correlation-{self._create_param_tag_correlation()}'):
            os.mkdir(
                f'metadata/correlation-{self._create_param_tag_correlation()}')
    ...
    def get_feature_json_path(self, layer: int, neuron: int):
        return f'metadata/correlation-{self._create_param_tag_correlation()}/layer_{layer}_neuron_{neuron}.json'
